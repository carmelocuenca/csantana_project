...
write_files:
  - path: "/etc/fleet_machines.env"
    ...
    encoding: "base64"
    content: |
      UE9TVEdSRVNfVVNFUj1wb3N0Z3Jlcw0KUE9TVEdSRVNfUEFTU1dPUkQ9cG9zdGdyZXM=
  - path: "/etc/nginx.conf"
    permissions: "0644"
    content: |
      server {
...
Como ya no es necesario el almacenamiento NFS compartido ya no hay que realizar la copia de los ficheros de las unidades del entorno local a la máquina CoreOS. Por lo tanto ya no es necesaria la línea de provisión del script en el fichero \kode{Vagranfile}.

El fichero user-data es validado con la utilidad \textit{Config Validator}\footnotettt{Config Validator}{Config Validator}. 

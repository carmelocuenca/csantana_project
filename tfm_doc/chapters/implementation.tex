...
\subsection{Uso del servicio AWS Identity and Access Management (IAM)}

Se hace uso de este servicio AWS con la intención de controlar de forma segura el acceso a servicios y recursos por parte de los usuarios. Así, el primer paso será la protección de las claves de acceso a la cuenta creada. Esto es necesario puesto que se van a realizar peticiones a los servicios AWS desde un programa que accederá, a su vez, a través de una API. De esta manera es necesario disponer de una pareja de claves de acceso, conocidas como \textit{access key ID} y \textit{secret access key}. Estas claves de acceso \textit{root} permiten acceder ilimitadamente a todos los recursos y sobre ella no se pueden restringir los permisos asociados. Por ello es necesario crear usuarios con claves de acceso a los que se les asignarán, únicamente, los permisos que necesiten. Así, se creará lo siguiente con el mínimo número de pivilegios necesarios para acceder a los recursos:
\begin{itemize}
\item Grupo \textit{administrators} con permiso \textit{Administrator Access} para disponer de acceso completo a los recursos y servicios de la cuenta AWS.
\item Usuario \textit{admin} con tipo de acceso \textit{Programmatic access} y perteneciente al grupo \textit{administrators}.
\end{itemize}

Al finalizar, las credenciales del usuario \textit{admin} podrán ser descargadas y almacenadas en un lugar oculto con permisos de lectura y escritura para el propietario del fichero:

%= lang:bash
\begin{code}
$ mkdir ~/.aws-credentials
$ mv /home/carolina/Descargas/credentials.csv ~/.aws-credentials/
$ mv ~/.aws-credentials/credentials.csv ~/.aws-credentials/credentials-admin.csv
$ chmod 0600 ~/.aws-credentials/credentials-admin.csv
\end{code}

Para poder acceder con el usuario \textit{admin} por consola gráfica se le crea una contraseña, distinta a las claves programáticas. Para ello, dentro del usuario \textit{admin}, en el servicio IAM hay que dirigirse a \textit{Console Password} y editar en \textit{Manage Password}. Así, se habilita el acceso por consola y se crea una contraseña para el propósito.

Además, se customiza la url para iniciar sesión de ahora en adelante en el panel del servicio IAM.

A partir de ahora cuando se quieran realizar operaciones de administración se trabajará desde el usuario \textit{admin}.

Con el objetivo de tener un usuario para solamente actividades de desarrollo se crea lo siguiente:
\begin{itemize}
\item Grupo \textit{deployers} con permiso para manejar las instancias EC2. 
\item Usuario \textit{deployer} con tipo de acceso \textit{AWS Management Console access}, facilitando una contraseña, y perteneciente al grupo \textit{administrators}.
\end{itemize}

Para realizar operaciones de desarrollo se iniciará sesión en la cuenta AWS con el usuario deployer.

\subsection{Uso del servicio Virtual Private Cloud (VPC)}

Este servicio permite controlar todos los aspectos del entorno de red virtual y será usado con la intención de hacer la selección del rango de direcciones IP que se les dará a las máquinas virtuales y la creación de una subred acorde.
...
\subsection{Uso del servicio Elastic Cloud Computing (EC2)}
...
Lo primero se realiza accediendo al servicio EC2 y navegando hasta \textit{Network \& Security, Key Pairs}. Se crea una nueva pareja de claves con el nombre \kode{keypair.pem}, la cual se guarda en el directorio \kode{\textasciitilde{}/.aws-credentials}, facilitándole los permisos de lectura y escritura al propietario:
...
Para poder llevar a cabo esta parte se necesita una nueva configuración \kode{cloud-config}. Este nuevo \kode{user-data} recibirá el nombre de \kode{user-data.sampleapp.aws}. Este fichero tendrá, únicamente, dos diferencias respecto al anterior. La primera será especificar al servicio flannel que ha de usar el rango de direcciones IPv4 privadas, pues los contenedores internos a la máquina usarán una red privada. El segundo cambio detalla que la red privada a usar será la 172.17.0.0/16 y el protocolo UDP.
...
\begin{itemize}
\item \kode{access\_key\_id}: Primera clave, perteneciente al identificador del usuario.
\item \kode{secret\_access\_key}: Segunda clave, perteneciente a la contraseña secreta.
\item \kode{keypair\_name}: Nombre del archivo que contiene el par de claves.
\item \kode{aws\_region}: Región.
\item \kode{aws\_availability\_zone}: Zona de disponibilidad.
\item \kode{aws\_subnet\_id}: Identificador de la subred.
\item \kode{aws\_security\_groups}: Identificador del grupo de seguridad.
\item \kode{aws\_ami}: Identificador de la AMI a usar. Una AMI es una \textit{Amazon Machine Image}. Como en la iteración anterior se está usando la versión CoreOS alpha 1339.0.0. Se escoge la misma aquí con el tipo de virtualización HVM: \textit{CoreOS-alpha-1339.0.0-hvm - ami-00598116} de 64 bits.
\item \kode{aws\_instance\_type}: Tipo de instancia a lanzar. El tipo será \textit{t2.micro Free tier eligible} al ser gratuita.
\item \kode{aws\_elastic\_ip}: Indicación de uso de direcciones IP elásticas para el acceso remoto al servicio.
\item \kode{private\_ip\_adresses}: Dirección IP privada a asignar a la instancia.
\item \kode{user\_data}: Contenido del fichero \kode{user-data}.
\end{itemize}
...
Así, el fichero \kode{Vagrantfile} queda de la siguiente manera:
...
\caption{Comprobación de la red virtual para contenedores con 1 máquina virtual.\label{fig:figure_placement_example}}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/figures/curl-aws-1.png}
...
Luego se accede a las instancias, desde diferentes terminales, donde X puede ser 1, 2 o 3:
...
Ahora se prueba que la salud del cluster es positiva, por ejemplo en la máquina virtual core-02:
...
\caption{Comprobación de la salud del cluster con 3 máquinas virtuales.\label{fig:figure_placement_example}}
\end{figure}

Además, se comprueba que la red virtual que une a los contenedores mediante direcciones IP virtuales funciona correctamente:
...
\caption{Comprobación de la red virtual para contenedores. 3 máquinas virtuales.\label{fig:figure_placement_example}}
\end{figure}
...
Por último se comprueba desde la consola de comandos que se puede acceder al servicio correctamente con el comando \kode{curl}. Por ejemplo desde la máquina virtual core-02:
...
Además, se utiliza la IP elástica de la misma máquina para acceder a través del navegador web:
